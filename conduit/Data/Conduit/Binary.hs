...
conduitFile :: MonadResource m
            => FilePath
            -> Conduit S.ByteString S.ByteString m ()
...
--
-- Since 0.3.0
takeWhile :: Monad m => (Word8 -> Bool) -> Conduit S.ByteString S.ByteString m ()
takeWhile p =
    loop
  where
...
--
-- Since 0.3.0
lines :: Monad m => Conduit S.ByteString S.ByteString m ()
lines =
    loop id
  where
...
--
-- Since 0.5.0
sourceLbs :: Monad m => L.ByteString -> Source S.ByteString m ()
